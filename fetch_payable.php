<?php
	...
	$str_sql = "SELECT * FROM payable_tb "; 

	if($_POST['query'] != '') {

		$str_sql .= 'WHERE paya_name LIKE "%'.str_replace(' ', '%', $_POST['query']).'%"  ';

	}

	$str_sql .= 'ORDER BY paya_id DESC ';
	...
	if($total_data > 0) {
			...
			$output .= '<tr>
							...
							<td>
								<div class="btn-group btn-group-toggle">
									<button type="button" name="edit" id="'.$obj_row["paya_id"].'" class="btn btn-warning edit_data" title="แก้ไข / Edit">
										<i class="icofont-edit"></i>
									</button>

									<button type="button" name="view" id="'.$obj_row["paya_id"].'" class="btn btn-primary view_data" title="ดู / View">
										<i class="icofont-eye-alt"></i>
									</button>

									<button type="button" name="delete" id="'.$obj_row["paya_id"].'" class="btn btn-danger delete_data" title="ลบ / Delete">
		...
		$output .= '</tbody>
					<tbody>
						<tr>
							<td colspan="3" align="center">ไม่มีข้อมูลบริษัทเจ้าหนี้</td>
						</tr>
					</tbody>';
